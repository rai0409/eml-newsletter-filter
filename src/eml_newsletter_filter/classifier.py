...
def classify_email(email: ParsedEmail, config: dict) -> ClassificationResult:
    ...
    # Exclusions deliberately precede force_newsletter to protect transactional mail.
    if exclude:
    ...
    features, reasons = detect_features(email, int(config["classification"]["many_links_threshold"]))
    score = sum(int(config["feature_weights"].get(feature, 0)) for feature in features)
    add_keywords = find_keywords(text, keywords["add_as_newsletter"])
    if add_keywords:
        score += int(config["classification"]["add_keyword_score"])
    ...
    elif score >= int(config["classification"]["newsletter_threshold"]): classification = "newsletter"
    elif score >= int(config["classification"]["suspected_threshold"]): classification = "suspected_newsletter"
